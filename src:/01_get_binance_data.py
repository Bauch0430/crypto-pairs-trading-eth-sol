...
CONFIG = {
    'exchange_id': 'binance',
    'symbols': {
        'ETH': 'ETH/USDT:USDT',  # Binance 合約代碼格式
        'SOL': 'SOL/USDT:USDT'
    },
    'timeframe': '1d',           # 日線
    'start_date': '2022-01-01 00:00:00',
    'output_file': 'pair_merged_1d_real.parquet' # 區分一下檔名
}

def init_exchange():
    # 不需要 API Key 也能抓公開歷史數據
    return ccxt.binance({
...
def fetch_perp_ohlcv(exchange, symbol, timeframe, start_str):
    ...
    return df[['close']] # 只留收盤價

# ==========================================
# 3. 抓取 Funding Rate (關鍵步驟)
# ==========================================
def fetch_funding_daily(exchange, symbol, start_str):
    ...
    while True:
        try:
            ...
            print(f"   -> 已抓取 {len(all_funding)} 筆費率...")
    ...
    if not all_funding:
        print("❌ 警告：抓不到 Funding Rate，請確認網路或代碼是否正確。")
        return pd.DataFrame()
    ...
    # 【關鍵】每日加總：把一天 3 次的費率加起來，變成當日持有成本
...
def main():
    ...
    for name, symbol in CONFIG['symbols'].items():
        # A. 抓 K 線
        df_price = fetch_perp_ohlcv(exchange, symbol, CONFIG['timeframe'], CONFIG['start_date'])
        
        # B. 抓 Funding Rate
        df_fund = fetch_funding_daily(exchange, symbol, CONFIG['start_date'])
        
        # C. 合併 (Left Join 以防 Funding 缺漏)
        # 將 Funding 欄位改名，例如 ETH_funding
        df_fund.rename(columns={'fundingRate': 'daily_funding_sum'}, inplace=True)
        
        df_merged = df_price.join(df_fund, how='left')
        df_merged['daily_funding_sum'] = df_merged['daily_funding_sum'].fillna(0)
    ...
    pair_df = pd.concat([eth, sol], axis=1, join='inner')
    
    # --- 計算 Log Price & Returns ---
    ...
    print(f"\n🎉 成功！真實數據已儲存為: {CONFIG['output_file']}")
    print(pair_df.tail())
